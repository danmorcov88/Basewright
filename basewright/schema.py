"""Reading the schema documents, and turning a violation into something actionable.

Every document this project reads from disk is described by a closed JSON Schema: the
seven files of a profile, the facts a collector produces, and the plan artifact. They are
all validated by the code here, so a refusal reads the same whichever of them was wrong.

Closing every object is the mechanism the architecture rests on rather than a matter of
taste. A document that cannot carry a key the core does not read is a document that cannot
imply behaviour the core does not have.

Schemas live in ``schema/`` at the repository root, where they are reviewable next to the
documents they describe, and are copied into the wheel so an installed Basewright validates
without a checkout.
"""
...
_CANDIDATES = (
    Path(__file__).resolve().parent / "_schema",
...
_GENERIC_HINTS: dict[str, str] = {
    "additionalProperties": (
        ...
        "letting a document smuggle it in."
    ),
...
def problems_in(document: object, *, schema_name: str, file: str) -> list[Problem]:
...
def _problems_from(error: ValidationError, file: str) -> Iterator[Problem]:
